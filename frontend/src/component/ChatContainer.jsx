    ...
    <div className="w-full h-full bg-black rounded-lg text-white p-4 md:p-6 flex flex-col">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1">
          <h2 className="text-base md:text-lg font-semibold">{selectedUser?.fullName}</h2>
          <p className="text-xs text-green-400">Online</p>
        </div>
        <div className="hidden sm:flex gap-4 text-gray-400 text-lg">
  ...
  return (
    ...
    >
      ...
      >
        ...
        {i === messages.length - 1 && <div ref={lastMsgRef} />}
      </div>

      {isSentByMe && (
        <img
          src={JSON.parse(localStorage.getItem("user"))?.profilePic || avatar}
          alt="sender"
          className="w-8 h-8 rounded-full object-cover"
        />
      )}
